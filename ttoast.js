function TToast (message,mode = 'success',costumize = {iconSize : '',roundedClass : '',textClass : '',duration: null,animation: ''}){
  ...
  switch (mode) {
    ...
    case 'danger':
      ...
      break;
    default: break;}  
  let html = `<div class="flex items-center bg-${color}-${mode == 'info' || mode == 'warning' ? '400' : '500'} border-l-4 border-${color}-700 py-2 px-3 shadow-md mb-2 ${costumize.roundedClass} ${costumize.animation}"><div class="text-${color}-${color == 'info' || color == 'warning' ? '400' : '500'} rounded-full bg-white mr-3">
  ...
  setTimeout(() => toast.removeChild(toast.firstChild), costumize.duration || 3000);
}